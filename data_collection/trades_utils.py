...
def download_trades(symbol: str, date_str: str, out_base: str = "trades_data", delete_zip=True, session=None):
...
def download_trades_range(symbols, period: str, out_base: str = "trades_data", pause_seconds: float = 0.15):
    ...
    if start_date > end_date:
        raise ValueError("Invalid period: after adjusting for latest available date, no past days remain.")
    ...
    for symbol in result["requested_symbols"]:
        for date_str in _date_list(start_date, end_date):
            ...
            try:
                extracted = download_trades(symbol, date_str, out_base=out_base, delete_zip=True, session=session)
